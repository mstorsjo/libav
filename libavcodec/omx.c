...
static av_cold int omx_decode_init(AVCodecContext *avctx)
{
    ...
    if (avctx->extradata_size && (avctx->codec->id != AV_CODEC_ID_H264 || avctx->extradata[0] != 1)) {
        ...
        if (avctx->codec_id == AV_CODEC_ID_WMV3 && avctx->extradata_size >= 4 &&
            buffer->nAllocLen >= 36) {
            int profile;
            // According to OMX IL 1.2.0 spec (4.3.33.2), the codec config
            // data for VC-1 Main/Simple (aka WMV3) is according to table 265
            // in the VC-1 spec. Most of the fields are just set with dummy values
            ...
            static const uint8_t wmv3seq[] = {
                ...
                0x00, 0x00, 0x00, 0x00, // struct C, almost equal to extradata
            ...
            memcpy(&buffer->pBuffer[8], avctx->extradata, 4);
...
static const AVClass omx_wmv3dec_class = {
    .class_name = "wmv3_omx_dec",
    .item_name  = av_default_item_name,
    .option     = options,
    .version    = LIBAVUTIL_VERSION_INT,
};
AVCodec ff_wmv3_omx_decoder = {
    .name             = "wmv3_omx",
    .long_name        = NULL_IF_CONFIG_SMALL("OpenMAX IL WMV3 video decoder"),
    .type             = AVMEDIA_TYPE_VIDEO,
    .id               = AV_CODEC_ID_WMV3,
    .priv_data_size   = sizeof(OMXCodecContext),
    .init             = omx_decode_init,
