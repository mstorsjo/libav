                               ...
                               const uint8_t *data, int size, int *ref)
{
    ...
    if ((ret = update_size(avctx, w, h)) < 0) {
        av_log(avctx, AV_LOG_ERROR, "Failed to initialize decoder for %dx%d\n", w, h);
        return ret;
    }
                            ...
                            int *got_frame, const uint8_t *data, int size)
{
    ...
    s->cur_frame->pict_type = s->keyframe ? AV_PICTURE_TYPE_I : AV_PICTURE_TYPE_P;
        ...
        for (row = s->tiling.tile_row_start;
            ...
            if (row + 8 < s->rows) {
                memcpy(s->intra_pred_data[0],
                       s->cur_frame->data[0] + yoff + 63 * s->cur_frame->linesize[0],
                       8 * s->cols);
                memcpy(s->intra_pred_data[1],
                       s->cur_frame->data[1] + uvoff + 31 * s->cur_frame->linesize[1],
                       4 * s->cols);
                memcpy(s->intra_pred_data[2],
                       s->cur_frame->data[2] + uvoff + 31 * s->cur_frame->linesize[2],
                       4 * s->cols);
            }
