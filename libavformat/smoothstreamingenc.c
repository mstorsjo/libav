...
static int64_t ism_seek(void *opaque, int64_t offset, int whence)
{
    ...
    for (i = os->nb_fragments - 1; i >= 0; i--) {
        ...
        if (offset >= frag->start_pos && offset < frag->start_pos + frag->size) {
            ...
            ret = ffurl_open(&os->out, frag->file, AVIO_FLAG_READ_WRITE, &os->ctx->interrupt_callback, &opts, NULL);
            av_dict_free(&opts);
            if (ret < 0) {
                os->out = os->tail_out;
                os->tail_out = NULL;
            ...
            av_dict_set(&opts, "truncate", "0", 0);
            ffurl_open(&os->out2, frag->infofile, AVIO_FLAG_READ_WRITE, &os->ctx->interrupt_callback, &opts, NULL);
            av_dict_free(&opts);
...
static int ism_flush(AVFormatContext *s, int final)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        ret = ffurl_open(&os->out, filename, AVIO_FLAG_WRITE, &s->interrupt_callback, NULL, NULL);
        if (ret < 0)
            break;
