...
static void writeout(AVIOContext *s, const uint8_t *data, int len)
{
    if (!s->error) {
        ...
        if (s->write_data_type)
            ret = s->write_data_type(s->opaque, (uint8_t *)data, len,
                                     s->current_type,
                                     s->last_time);
...
void avio_write_marker(AVIOContext *s, int64_t time, enum AVIODataType type)
{
    if (!s->write_data_type)
        return;
    // If ignoring parse points, just treat it as opaque
    ...
    // data and setting the type to opaque
    if (type == AVIO_DATA_OPAQUE &&
        (s->current_type != AVIO_DATA_HEADER &&
         s->current_type != AVIO_DATA_TRAILER))
        return;
